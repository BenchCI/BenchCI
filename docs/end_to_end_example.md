...
This gives you a clean hardware baseline before adding CI variables such as workflow credentials, artifact paths, runner setup, or cloud scheduling.
...
6. BenchCI downloads results and evidence into `benchci-results/` by default
...
## Preflight diagnostics before CI

Before relying on GitHub Actions or another CI system, run the local readiness workflow from [Diagnostics](diagnostics.md) on the hardware-connected machine. Then start the cloud Agent and verify the bench appears as healthy or degraded in the dashboard.

A bench that is failing or unknown is not eligible for scheduled cloud runs.
