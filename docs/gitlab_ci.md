...
Most shared GitLab runners should leave `BENCHCI_MACHINE_ID` unset. Set it only for a persistent runner or a non-parallel pipeline lane that should always count as the same CI runner. If another active CI session already uses the same value, login is blocked until that session logs out or expires.
...
codes or authenticator secrets to GitLab variables; the `benchci login` command
...
hardware-test:
  ...
  script:
    ...
    - benchci login --email "$BENCHCI_EMAIL" --password "$BENCHCI_PASSWORD"
    - benchci run --cloud --bench-id "$BENCHCI_BENCH_ID" --suite suite.yaml --artifact build/firmware.elf --verbose
  # The CLI automatically logs out the CI session after benchci run --cloud completes,
  # freeing the runner slot immediately regardless of run outcome.
  artifacts:
    when: always
    paths:
      - benchci-results/
...
hardware-test:
  ...
  script:
    ...
    - benchci run --cloud --bench-id "$BENCHCI_BENCH_ID" --suite suite.yaml --firmware-url "$SIGNED_FIRMWARE_URL" --firmware-sha256 "$FIRMWARE_SHA256" --verbose
```
...
Before enabling GitLab CI hardware runs, verify the bench on the hardware-connected machine:
