...
New accounts start a 30-day trial of Team features after email verification. For commercial
usage after the trial, contact `tech@benchci.dev`.
...
`benchci doctor` automatically inspects available serial ports, USB devices,
GPIO chips, Python modules, and common flashing tools:
...
Python backend libraries are installed with the BenchCI package; this section covers tools, OS access, and device permissions outside the package.
