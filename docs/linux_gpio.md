...
In Cloud Mode, the cloud-connected Agent executes `run_local(...)` on the hardware-connected machine, so GPIO access still happens locally on the Agent machine. The user or CI runner does not need direct access to `/dev/gpiochipX`.

<!-- Unreleased hardware GPIO docs are intentionally hidden until release. -->
