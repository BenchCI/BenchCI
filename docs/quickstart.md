...
BenchCI records the seed, step index, first failing case, and a JSONL case log in `logs/fuzz/`, so failures can be replayed. See [Protocol Fuzzing](fuzzing.md) before adding fuzzing to CI gates.
...
Before the first real run, use the readiness workflow:
...
Then run the actual suite:

```bash
benchci run   --bench bench.yaml   --suite suite.yaml   --artifact build/fw.elf
```

Use this order to separate problems:

```text
validate        -> YAML or suite/bench compatibility issue
self-test       -> bench infrastructure issue
dry-run plan    -> execution plan surprise
run             -> firmware, test logic, or hardware behavior issue
```

See [Validation, Self-Test, and Dry-Run Planning](diagnostics.md) for the full workflow.
